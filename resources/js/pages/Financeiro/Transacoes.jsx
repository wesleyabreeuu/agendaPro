          ...
          <div className="space-y-6">
            <Box title="Filtros">
              <form onSubmit={submitFilter} className="grid gap-4">
                <select className="h-10 rounded-md border border-zinc-200 px-3 text-sm" value={filterForm.data.tipo} onChange={(e) => filterForm.setData('tipo', e.target.value)}>
                  <option value="">Todos os tipos</option>
                  <option value="receita">Receitas</option>
                  <option value="despesa">Despesas</option>
                </select>
                {financeiroAvancado ? (
                  <select className="h-10 rounded-md border border-zinc-200 px-3 text-sm" value={filterForm.data.status} onChange={(e) => filterForm.setData('status', e.target.value)}>
                ...
                <select className="h-10 rounded-md border border-zinc-200 px-3 text-sm" value={filterForm.data.categoria} onChange={(e) => filterForm.setData('categoria', e.target.value)}>
                  <option value="">Todas as categorias</option>
                  {categorias.map((categoria) => <option key={categoria.id} value={categoria.id}>{categoria.nome}</option>)}
                </select>
                <select className="h-10 rounded-md border border-zinc-200 px-3 text-sm" value={filterForm.data.conta} onChange={(e) => filterForm.setData('conta', e.target.value)}>
            ...
            <Box title="Nova categoria">
              <form onSubmit={submitCategoria} className="grid gap-4">
                <select className="h-10 rounded-md border border-zinc-200 px-3 text-sm" value={categoriaForm.data.tipo} onChange={(e) => categoriaForm.setData('tipo', e.target.value)}>
                ...
                <button className="inline-flex h-10 items-center justify-center rounded-md border border-zinc-200 bg-white px-4 text-sm font-medium text-zinc-900">Salvar categoria</button>
              </form>
            </Box>
            ...
            <Box title="Novo lançamento">
              <form onSubmit={submitTransacao} className="grid gap-4">
                ...
                <label className="inline-flex items-center gap-3 text-sm text-zinc-700">
                  <input type="checkbox" checked={transacaoForm.data.recorrente} onChange={(e) => transacaoForm.setData('recorrente', e.target.checked)} />
                  <span>Lançamento recorrente</span>
                </label>
                {transacaoForm.data.recorrente ? (
                  <select className="h-10 rounded-md border border-zinc-200 px-3 text-sm" value={transacaoForm.data.frequencia} onChange={(e) => transacaoForm.setData('frequencia', e.target.value)}>
                    ...
                    <div className="text-right">
                      ...
                      <div className="mt-3 flex justify-end gap-2">
                        {financeiroAvancado && tx.status === 'pendente' ? (
                          <button type="button" onClick={() => router.patch(`/financeiro/transacoes/${tx.id}/settle`, { conta_bancaria_id: tx.conta?.id, forma_pagamento: 'conta', data: tx.data_iso })} className="rounded-md border border-green-200 px-3 py-2 text-xs text-green-700">
                        ...
                        <button type="button" onClick={() => router.delete(`/financeiro/transacoes/${tx.id}`)} className="rounded-md border border-red-200 px-3 py-2 text-xs text-red-600">Excluir</button>
                      </div>
                    </div>
                  </div>
                </div>
