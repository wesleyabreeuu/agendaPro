      ...
      <div className="space-y-6">
          ...
          <div className="flex flex-wrap items-center justify-between gap-4">
            ...
            <div className="flex flex-wrap gap-3">
              <Button asChild className="w-auto">
                <Link href="/rotinas/hoje">Abrir rotinas de hoje</Link>
              </Button>
              <Button asChild variant="outline" className={`w-auto ${isDark ? 'border-zinc-700 bg-zinc-950 text-zinc-100 hover:bg-zinc-900 hover:text-zinc-50' : ''}`}>
                <Link href="/rotinas/templates">Ver templates</Link>
              </Button>
            </div>
          </div>
        </section>
        ...
        <div className="grid gap-6 xl:grid-cols-[1.15fr_0.85fr]">
          <Panel
            ...
            action={<Button asChild variant="outline" size="sm" className={`w-auto ${isDark ? 'border-zinc-700 bg-zinc-950 text-zinc-100 hover:bg-zinc-900 hover:text-zinc-50' : ''}`}><Link href="/rotinas/hoje">Ir para execução <ArrowRight className="h-3.5 w-3.5" /></Link></Button>}
            isDark={isDark}
          >
            <div className={`rounded-xl border p-4 ${isDark ? 'border-zinc-700 bg-zinc-950' : 'border-zinc-200 bg-zinc-50/70'}`}>
              <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <div className={`rounded-lg border px-4 py-3 text-right ${isDark ? 'border-zinc-700 bg-zinc-900' : 'border-zinc-200 bg-white'}`}>
            ...
            <div className="mt-4 space-y-3">
              {(today.items || []).slice(0, 5).map((item) => (
                <div key={item.id} className={`rounded-lg border px-4 py-3 ${isDark ? 'border-zinc-700 bg-zinc-950' : 'border-zinc-200 bg-white'}`}>
              ...
              {!today.items?.length ? (
                <div className={`rounded-lg border border-dashed px-4 py-8 text-sm ${isDark ? 'border-zinc-700 text-zinc-400' : 'border-zinc-300 text-zinc-500'}`}>Nenhuma rotina prevista para hoje.</div>
              ) : null}
            </div>
          </Panel>
          ...
          <Panel title="Progresso recente" subtitle="Leitura dos últimos dias para enxergar constância." isDark={isDark}>
            <div className="space-y-3">
              {recentProgress.map((day) => (
                <div key={day.data} className={`rounded-lg border px-4 py-3 ${isDark ? 'border-zinc-700 bg-zinc-950' : 'border-zinc-200 bg-white'}`}>
        ...
        <div className="grid gap-6 xl:grid-cols-[0.95fr_1.05fr]">
          <Panel title="Áreas da vida" subtitle="Distribuição das rotinas ativas e previstas hoje." isDark={isDark}>
            <div className="space-y-3">
              {categoryBreakdown.map((item) => (
                <div key={item.categoria} className={`rounded-lg border px-4 py-3 ${isDark ? 'border-zinc-700 bg-zinc-950' : 'border-zinc-200 bg-white'}`}>
                  <div className="flex items-center justify-between gap-3">
                    <span className="rounded-full border px-2.5 py-1 text-xs font-medium" style={categoryBadgeStyle(item.categoria, isDark)}>{categoryLabel(item.categoria)}</span>
          ...
          <Panel title="Atalhos rápidos" subtitle="Ações úteis para ajustar seu sistema sem perder ritmo." isDark={isDark}>
            <div className="grid gap-4 md:grid-cols-2">
              <Link href="/rotinas/minhas" className={`rounded-xl border p-5 shadow-sm transition ${isDark ? 'border-zinc-700 bg-zinc-950 hover:bg-zinc-900' : 'border-zinc-200 bg-white hover:bg-zinc-50'}`}>
              ...
              <Link href="/rotinas/criar" className={`rounded-xl border p-5 shadow-sm transition ${isDark ? 'border-zinc-700 bg-zinc-950 hover:bg-zinc-900' : 'border-zinc-200 bg-white hover:bg-zinc-50'}`}>
              ...
              <Link href="/rotinas/historico" className={`rounded-xl border p-5 shadow-sm transition ${isDark ? 'border-zinc-700 bg-zinc-950 hover:bg-zinc-900' : 'border-zinc-200 bg-white hover:bg-zinc-50'}`}>
              ...
              <Link href="/rotinas/templates" className={`rounded-xl border p-5 shadow-sm transition ${isDark ? 'border-zinc-700 bg-zinc-950 hover:bg-zinc-900' : 'border-zinc-200 bg-white hover:bg-zinc-50'}`}>
