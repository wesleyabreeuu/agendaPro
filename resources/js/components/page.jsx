...
function PageCard({ className = '', children, ...props }) {
  return (
    <Card
      className={cn('rounded-xl border-zinc-200 bg-white shadow-sm dark:border-zinc-800 dark:bg-zinc-900', className)}
      {...props}
    >
      {children}
    </Card>
...
function PageEmptyState({ icon: Icon, title, description, action, className = '' }) {
  return (
    <div className={cn('flex flex-col items-center justify-center rounded-xl border border-dashed border-zinc-300 bg-white px-6 py-12 text-center dark:border-zinc-700 dark:bg-zinc-900', className)}>
      {Icon ? (
